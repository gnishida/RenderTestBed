...
void RenderManager::init(const std::string& vertex_file, const std::string& geometry_file, const std::string& fragment_file, bool useShadow, int shadowMapSize) {
	...
	void RenderManager::resizeFragTex(int winWidth,int winHeight){
...
void RenderManager::render(const QString& object_name) {
	for (auto it = objects[object_name].begin(); it != objects[object_name].end(); ++it) {
		...
		if (renderingMode == RENDERING_MODE_REGULAR) {
			...
			glUniform1f(glGetUniformLocation(program_pass1, "depthSensitivity"), depthSensitivity);
			glUniform1f(glGetUniformLocation(program_pass1, "normalSensitivity"), normalSensitivity);
			glUniform1i(glGetUniformLocation(program_pass1, "useThreshold"), useThreshold ? 1 : 0);
			glUniform1f(glGetUniformLocation(program_pass1, "threshold"), threshold);
		}
