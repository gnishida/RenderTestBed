...
namespace LC {
	...
	uint Shader::initShader(QString& vertFileName,QString& fragFileName,std::vector<QString>& fragData){
		//GLenum err;
		// read shaders
		std::string source;
		...
		// init fragment data location
		if(fragData.size()==0){
			glBindFragDataLocation(programId, 0, "outputF");
		}else{
			for(int i=0;i<fragData.size();i++){
				glBindFragDataLocation(programId,i, fragData[i].toAscii().constData());
			}
		}
